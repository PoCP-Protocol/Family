...
async function main() {
  ...
  process.stdout.write(
    `FELS reference HTTP API listening on ${started.port} (read-only, family_legacy)\n`,
  );
  const shutdown = () => {
...
main().catch((error) => {
  process.stderr.write(
    `FELS HTTP API failed to start: ${error instanceof Error ? error.message : String(error)}\n`,
  );
  process.exit(1);
});
