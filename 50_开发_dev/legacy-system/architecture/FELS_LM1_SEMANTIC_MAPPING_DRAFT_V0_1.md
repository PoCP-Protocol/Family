...
```text
STATUS = LM1_MAPPING_REVIEW_REQUIRED   (FLM-AC-002 撤销上轮 REVIEWED)
...
> ⚠️ 更正(FLM-AC-002, 2026-08-14):上轮据自签署把本草稿升为 `LM1_MAPPING_REVIEWED`,已**撤销**——因正文仍存未解决问题(M014 checkin→ActionCompletion/MIGRATE、M054/M055 直指 Causal、以及"需总架构师裁决"的开放问题),与 REVIEWED 自相矛盾。状态回 `REVIEW_REQUIRED`。纠正后的映射另立 `FELS_LM1_SEMANTIC_MAPPING_V1.md`,本 V0.1 保留为历史。
