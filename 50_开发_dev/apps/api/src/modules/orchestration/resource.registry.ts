 ...
 * PRACTICE 仅当有 approved Content Ref;EXTERNAL_REFERRAL 仅当配置真实目标(不臆造)。
 */
...
function practiceOffer(contentRef: string): ResourceOfferDto {
  return {
    offer_id: `${OFFER_IDS.PRACTICE}:${contentRef}`, resource_type: 'PRACTICE', qualification_mode: 'REQUIRED',
    provider_ref: SELF_AI_COACH_PROVIDER_REF, external_referral_target_ref: null,
...
/** 候选原子 Offer(先于 Eligibility)。稳定 ID;PRACTICE/EXTERNAL 仅当真实存在。 */
export function candidateOffersForCommunicationConflict(env: ResourceRegistryEnv = {}): ResourceOfferDto[] {
  ...
  if (env.approvedPracticeContentRef) offers.push(practiceOffer(env.approvedPracticeContentRef));
