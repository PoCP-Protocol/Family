...
describe('Principal Action Bridge E2E (M3-101A-C, real PostgreSQL)', () => {
  it('POSITIVE: accepting a NORMAL proposal starts the canonical intervention (episode + 7 actions); proposal is single-use', async () => {
    ...
    const proposal = await createNormalProposal(setup.familyId);
    ...
    expect(p.rows[0].accepted_by_actor_id).toBe('architect-1');
  ...
  it('NEGATIVE: unknown / cross-family proposal -> 404', async () => {
    ...
    const p = await createNormalProposal(bare);
    const unknown = await accept(bare, randomUUID(), { onboarding_id: randomUUID(), priority_id: randomUUID(), idempotency_key: 'idem-unknown' }, 'corr-unknown');
    expect(unknown.status).toBe(404);
    // 跨家庭:proposal 属 bare,却用另一个 family 的 URL → 404(防跨家庭枚举)
    const other = await createBareFamily();
    const cross = await accept(other, p.action_proposal_id!, { onboarding_id: randomUUID(), priority_id: randomUUID(), idempotency_key: 'idem-cross' }, 'corr-cross');
  ...
  it('NEGATIVE: non-bridgeable intervention -> 409, no growth writes', async () => {
    ...
    const res = await accept(bare, p.action_proposal_id!, { onboarding_id: randomUUID(), priority_id: randomUUID(), idempotency_key: 'idem-nonbridge' }, 'corr-nonbridge');
  ...
  it('NEGATIVE: missing canonical permission (bare family) -> 403, Growth zero writes, proposal stays PROPOSED', async () => {
    const bare = await createBareFamily();
    const p = await createNormalProposal(bare);
    const res = await accept(bare, p.action_proposal_id!, { onboarding_id: randomUUID(), priority_id: randomUUID(), idempotency_key: 'idem-noperm' }, 'corr-noperm');
...
async function createNormalProposal(familyId: string): Promise<{ action_proposal_id: string | null; session_id: string }> {
  const sRes = await fetch(`${baseUrl}/families/${familyId}/principal/sessions`, {
    method: 'POST', headers: principalHeaders('corr-proposal'), body: JSON.stringify({ subject_ref: 'child-1' }),
  });
  ...
  const mRes = await fetch(`${baseUrl}/families/${familyId}/principal/sessions/${sid}/messages`, {
    method: 'POST', headers: principalHeaders('corr-proposal'), body: JSON.stringify({ subject_ref: 'child-1', message: '孩子写作业总是拖拉磨蹭，我该怎么办' }),
  });
...
function accept(familyId: string, proposalId: string, body: Record<string, unknown>, correlationId: string): Promise<Response> {
  return fetch(`${baseUrl}/families/${familyId}/principal/proposals/${proposalId}/accept`, {
    method: 'POST', headers: principalHeaders(correlationId), body: JSON.stringify(body),
  });
}

function principalHeaders(correlationId: string): Record<string, string> {
  return { 'content-type': 'application/json', 'x-actor-id': 'architect-1', 'x-correlation-id': correlationId };
}
