...
/**
 * @typedef {{ apiBaseUrl: string, familyId: string, initialPage?: string, firstSliceApiMode?: 'disabled'|'synthetic-api', coreGrowthApiMode?: 'disabled'|'synthetic-api', platformSurfacesApiMode?: 'disabled'|'synthetic-api', authToken?: string, authActorId?: string }} TestLoopConfig
 */
/** @type {TestLoopConfig} */
...
export function createTestLoopApp(root, config = defaultTestLoopConfig) {
  ...
  function growthReport() { return `${visualReference('growth-02-ai-report', '家庭成长报告：综合评估、优势风险建议和推荐成长路径', [['ref-bottom-cta', 'core-plan', '生成个性化方案']])}${coreGrowthPanel('UI-08')}`; }
  function growthDailyTask() { return `${clearReference('daily-growth-task-reference-448x916.png', '今日成长任务：机器人提醒、三项任务、积分时长、本周完成度、连续打卡和完成今日任务', [['clear-bottom-cta', 'page-objects-complete-daily-task', '完成今日任务']], '448/916')}${weeklyPlanActionContext()}${firstSlicePanel('UI-09')}`; }
  function growthChild() { return `${clearReference('growth-child-assistant-reference-448x920.png', '成长小助手：欢迎 Banner、成长能量、四色活动卡、今日挑战、奖励和开始挑战', [['clear-bottom-cta', 'growth-daily-task', '开始挑战']], '448/920')}${coreGrowthPanel('UI-10')}`; }
