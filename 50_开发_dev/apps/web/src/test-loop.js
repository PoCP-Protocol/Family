...
export function createTestLoopApp(root, config = defaultTestLoopConfig) {
  ...
  const personalGrowthJourneyPanel = (card) => {
    ...
    return `<section class="by-personal-growth-journey" data-platform-surface="UI-11" data-ui11-journey-state="${journey.state}"><small>我们的成长旅程</small><h2>${journey.headline}</h2>${entries}<div><button class="by-btn by-btn-ghost" data-by="ui11-open-plan">查看 90 天成长计划</button><button class="by-btn full-primary" data-by="ui11-open-family-review">查看家庭回顾</button></div></section>`;
  };
  ...
  const pageAssistiveStatus = () => (page === 'growth-ranking' && platformSurfacesApiEnabled() && platformSurfacesLoadState === 'READY')
    ? '成长旅程已更新。你可以回看已经走过的几步。'
    : llmTextEquivalent;
