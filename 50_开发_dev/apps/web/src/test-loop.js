...
export function createTestLoopApp(root, config = defaultTestLoopConfig) {
  ...
  function familySharingDraftPanel() {
    ...
    return `<section class="by-family-sharing-draft" data-ui26-sharing-draft-state="${familySharingDraftState || 'READY'}"><small>家庭分享草稿</small><h2>先留给家庭，再慢慢决定</h2><p>可以写下一个你们正在尝试的小变化，或一次想继续记得的相处时刻。</p><p class="by-sharing-draft-status">${status}</p><div><button class="by-btn by-btn-ghost" data-by="ui26-return-exchange-feed">回到家庭交流</button>${familySharingDraftState === 'SAVED' ? '' : '<button class="by-btn full-primary" data-by="ui26-save-sharing-draft">记下分享草稿</button>'}</div></section>`;
  }
