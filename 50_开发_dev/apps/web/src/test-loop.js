...
/**
 * @typedef {{ apiBaseUrl: string, familyId: string, initialPage?: string, firstSliceApiMode?: 'disabled'|'synthetic-api', coreGrowthApiMode?: 'disabled'|'synthetic-api', platformSurfacesApiMode?: 'disabled'|'synthetic-api', commerceCatalogApiMode?: 'disabled'|'synthetic-api', authToken?: string, authActorId?: string }} TestLoopConfig
 */
/** @type {TestLoopConfig} */
...
export function createTestLoopApp(root, config = defaultTestLoopConfig) {
  ...
  function familyContentDetailPanel() {
    ...
    return `<section class="by-family-content-detail" data-ui14-detail-state="${detailInterestState || 'READY'}" data-ui14-product-ref="${selectedCatalogItem.product_ref}"><small>家庭内容详情</small><h2>${selectedCatalogItem.title}</h2><p class="by-detail-version">内容版本 ${selectedCatalogItem.product_version || 1}</p>${state}<button class="by-btn full-primary" data-by="ui14-save-interest">记下了解意向</button><button class="by-btn by-btn-ghost" data-by="ui14-return-catalog">回到内容目录</button></section>`;
  }
  function commerceProduct() { return `${clearReference('product-detail-reference-418x970.png', '商品详情：21天亲子沟通挑战营、价格、服务权益、邀请优惠券和购买拼团操作区', [['clear-product-buy', 'commerce-submit-intent', '立即购买'], ['clear-product-group', 'llm-commerce-group', '发起拼团']], '418/970')}${familyContentDetailPanel()}`; }
  function commerceInvite() { return clearReference('invite-rewards-reference-432x992.png', '邀请有礼：邀请3个家庭、1/3进度、奖励卡、立即邀请、邀请方式和二维码横幅', [['clear-invite-cta', 'experience-create-invite', '确认邀请']], '432/992'); }
  function commerceGroup() { return clearReference('group-buy-reference-440x960.png', '拼团专区：分类Tab、四张拼团卡、团长、倒计时、参与头像、原价拼团价和去拼团按钮', [['clear-group-join', 'experience-create-group', '确认拼团']], '440/960'); }
