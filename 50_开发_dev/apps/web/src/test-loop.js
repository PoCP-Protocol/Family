...
export function createTestLoopApp(root, config = defaultTestLoopConfig) {
  ...
  function journeyPlanLivePanel() {
    ...
    const action = plan.status === 'DRAFT' ? '<button class="by-btn full-primary" data-by="ui05-confirm-journey-plan">由家庭确认并开始第 1 阶段</button>' : '';
    const receipt = journeyPlanActionState ? `<output data-ui05-journey-receipt="${journeyPlanActionState}">${journeyPlanActionState === 'CONFIRMED' ? '计划已由家庭确认；下一阶段需复盘后再决定。' : '计划草稿已保存，仍可由家庭决定是否确认。'}</output>` : '';
